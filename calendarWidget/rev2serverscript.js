(function() {
    ...
    data.CONST = {
        TEXT: {
            ADD_ALL_EVENTS: gs.getMessage('Add all to calendar') // Ensures the message is available
        }
    };
    ...
    function calculateSpans(items, schedule, start, end) {
        ...
        while (timeSpan.next()) {
            var spanTZ = timeSpan.schedule && timeSpan.schedule.time_zone 
                ? timeSpan.schedule.time_zone 
            ...
            for (var i = 0; i < timeSpans.length; i++) {
                ...
                items.push({
                    ...
                    yearText: yearText,
                    sysId: calSysId,
                    region: timeSpan.u_region.toString().trim().toLowerCase()
                });
            }
        }
    }
    ...
    function getRegions() {
        ...
        while (gr.next()) {
            ...
            var regionNameDisplay = gr.u_region.toString().trim(); 
            
            if (regionNameNormalized !== '' && !addedRegions[regionNameNormalized]) {
                data.regions.push({
                    sys_id: gr.sys_id.toString(),
                    u_region: regionNameDisplay, 
                    normalized_region: regionNameNormalized
                });
